...
public struct MultilineConverter: PlainConverter {
	...
	public func convert (_ record: Record<String, StandardRecordDetails>) -> String {
		...
		if case let .enabled(timestamp: isTimestampEnabled, _, _, _, _, _) = metaInfoEnabling, isTimestampEnabled {
		...
		if case let .enabled(_, level: isLevelEnabled, _, _, _, _) = metaInfoEnabling, isLevelEnabled {
